...
functionAxis.set_xlim([-0.005,4.505])
...
for value in Values:
	...
	if value==0:
		...
		plt.plot(X,Y, style[i], label=str(constant2[0]) + ' = ' + str(value))
		i += 1


plt.legend(loc=4, numpoints=1)
if len(Values) == 1:
	FILE_NAME =  directory + '2-' + constant1[2] + str(round(value1,2)) + str(constant2[2]) + str(round(Values[0],2))
else:
	FILE_NAME =  directory + '2-' + constant1[2] + str(round(value1,2))
